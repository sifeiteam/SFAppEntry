...
- (instancetype)init {
    self = [super init];
    if (self) {
        //强制启动startupWhenAppLaunched返回YES的组件
        [self startupComponents];
    }
    return self;
}

- (void)startupComponents {
    int classesCount = INT_MAX;
    Class SFComponentClass = NSClassFromString(@"SFComponent");
    Class *classes = NULL;
    ...
    for (int i = 0; i < classesCount; i++) {
        ...
        if (supercls != SFComponentClass) {
            continue;
        }
        BOOL start = ((BOOL (*)(id, SEL))(void *)objc_msgSend)(cls, @selector(startupWhenAppLaunched));
        if (!start) {
            continue;
        }
        [[SFComponentManager sharedInstance] startupComponentWithName:NSStringFromClass(cls)];
    }
    free(classes);
}
